...
class Uploader
{
    ...
    function __construct($uploadName = null) {
        ...
        if (isset($_FILES[$this->uploadName])) {
            ...
            if ($_FILES[$this->uploadName]['error'] === UPLOAD_ERR_OK) {
                ...
                return json('error', $this->errorCodeToMsg($_FILES[$this->uploadName]['error']));
            }
        ...
        if ($this->fileName) {
            ...
            $media_type = getMediaType($pathinfo['extension']);
            $this->uploadDir = storage_path('media/' . $media_type);
    ...
    public function handleUpload($uploadDir = null, $allowedExtensions = null, $file_size = null) {
        if (!$this->fileName) {
            return json('error', 'Incorrect upload name or no file uploaded');
            return false;
        }
        ...
        if ($this->fileSize > $this->sizeLimit) {
            return json('error', 'File size exceeds limit');
            return false;
        }
        ...
        if (!file_exists($this->uploadDir)) {
          mkdir($this->uploadDir, 0777);
            ...
            return json('error', 'Upload directory exists, but is not writable');
            return false;
        }
        ...
        if (file_exists($this->uploadDir . $this->fileName)) {
          ...
          $this->fileName = str_replace($ext, '', $this->fileName) . '-' . time() . $ext;
        }
        $this->savedFile = $this->uploadDir . $this->fileName;

        if (!$this->save($this->savedFile)) {
            return json('error', 'File could not be saved');
            return false;
        }
